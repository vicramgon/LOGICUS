module Modules.A_Expressions exposing (A_Expr, evaluateAExpr, parseAExpr, toStringAExpr, expressionA)
...
varsInA_Expr expr = case expr of
    Number i -> []
    Var s -> [s]
    Add e1 e2 -> varsInA_Expr e1 ++ varsInA_Expr e2
    Dif e1 e2 -> varsInA_Expr e1 ++ varsInA_Expr e2
    Mul e1 e2 -> varsInA_Expr e1 ++ varsInA_Expr e2
    Div e1 e2 -> varsInA_Expr e1 ++ varsInA_Expr e2
    Mod e1 e2 -> varsInA_Expr e1 ++ varsInA_Expr e2
    Error s -> []
    
        
evaluateAExpr: A_Expr -> List (String, Int) -> Maybe (Int)
evaluateAExpr expr ls = case expr of
    Number i -> Just i
    Var s -> case head <| filter (\x -> s == first x) ls of
        Nothing ->Nothing

        Just t -> Just (second t)
             
    Add e1 e2 -> if isNothing (evaluateAExpr e1 ls) ||  isNothing (evaluateAExpr e2 ls) then
                    Nothing
                 else
                    Just (withDefault 0 (evaluateAExpr e1 ls) + withDefault 0 (evaluateAExpr e2 ls))
    
    Dif e1 e2 -> if isNothing (evaluateAExpr e1 ls) ||  isNothing (evaluateAExpr e2 ls) then
                    Nothing
                 else
                    Just (withDefault 0 (evaluateAExpr e1 ls) - withDefault 0 (evaluateAExpr e2 ls))
    
    Mul e1 e2 -> if isNothing (evaluateAExpr e1 ls) ||  isNothing (evaluateAExpr e2 ls) then
                    Nothing
                 else
                    Just (withDefault 0 (evaluateAExpr e1 ls) * withDefault 0 (evaluateAExpr e2 ls))

    Div e1 e2 -> if isNothing (evaluateAExpr e1 ls) ||  isNothing (evaluateAExpr e2 ls) then
                    Nothing
                 else
                    Just (withDefault 0 (evaluateAExpr e1 ls) // withDefault 0 (evaluateAExpr e2 ls))
    
    Mod e1 e2 -> if isNothing (evaluateAExpr e1 ls) ||  isNothing (evaluateAExpr e2 ls) then
                    Nothing
                 else
...
expressionA = termAExpr |> andThen (expressionAAux []) 

type A_Operator = AddOp | DifOp | MulOp | DivOp | ModOp
...
expressionAAux revOps aExpr= oneOf 
                                [
                                    succeed Tuple.pair
                                      |= operatorA
                                      |= expressionA
...
toStringAExpr aExpr =
    case aExpr of
        Number p-> String.fromInt p
        Var p -> p
        Add p q -> (toStringAExpr p) ++ " + "  ++ (toStringAExpr q) 
        Dif p q -> (toStringAExpr p) ++ " - "  ++ (toStringAExpr q)
        Mul p q -> (toStringAExpr p) ++ " · "  ++ (toStringAExpr q)
        Div p q -> (toStringAExpr p) ++ " / "  ++ (toStringAExpr q) 
        Mod p q ->  (toStringAExpr p) ++ " mod "  ++ (toStringAExpr q)
        Error p -> "Error: " ++ p 
